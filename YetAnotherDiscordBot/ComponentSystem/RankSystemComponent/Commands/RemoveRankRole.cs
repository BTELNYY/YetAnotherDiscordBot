...
{
    public class RemoveRankRole : Command
    {
        public override string CommandName => "removerankrole";
        ...
        public override void LegacyExecute(SocketSlashCommand command)
        {
            ...
            bool actionCompleted = component.RemoveRankRole((uint)level, targetRole.Id, roleAction, out string error);
            if (!actionCompleted)
            {
                EmbedBuilder builder = new EmbedBuilder();
